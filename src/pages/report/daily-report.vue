<route lang="yaml">
  ...
  const openView = (item: DailyReport) => {
    viewItem.value = { ...item, isFeedbackSaved: item.isFeedbackSaved || false };
    selectedItem.value = item; 
  };
  
  const saveData = async () => {
    if (selectedItem.value) {
      selectedItem.value.isFeedbackSaved = true;
      selectedItem.value.feedback = viewItem.value.feedback;
  
      // Update store dengan feedback terbaru
      await apiDailyReportStore.updateDailyReportFeedback(selectedItem.value.id_daily_report, {
        feedback: selectedItem.value.feedback,
        isFeedbackSaved: selectedItem.value.isFeedbackSaved,
      });
  
      viewItem.value.isFeedbackSaved = true; // Nonaktifkan inputan feedback setelah simpan
    }
  };
            ...
            <div class="row mt-4">
              <div class="col mb-3">
                ...
                <textarea
                  id="feedback"
                  class="form-control"
                  v-model="viewItem.feedback"
                  rows="4"
                  placeholder="Berikan feedback Anda di sini"
                  :disabled="viewItem.isFeedbackSaved"
                ></textarea>
              </div>
            </div>
            <!-- /Inputan Feedback -->
